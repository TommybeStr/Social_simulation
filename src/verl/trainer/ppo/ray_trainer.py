        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # 为“当前 dataloader 的 root 批”生成 uid（后续 advantage 依据 uid 关联）
                    batch.non_tensor_batch["uid"] = np.array(
                    [str(uuid.uuid4()) for _ in range(len(batch.batch))], dtype=object
                    )

                    # === 识别 BFS 扩批 ===
                    root_bs = int(batch.batch.batch_size[0])                    # dataloader 提供的 root 数
                    gen_bs  = int(gen_batch_output.batch.batch_size[0])         # rollout 产出的样本数（BFS 后）
                    expected_n = int(getattr(self.config.actor_rollout_ref.rollout, "n", 1) or 1)

                    bfs_expansion = (gen_bs != root_bs * expected_n)

                    if not bfs_expansion:
                        # —— 常规：每个 root 产出固定 n —— #
                        batch = batch.repeat(repeat_times=expected_n, interleave=True)
                        batch = batch.union(gen_batch_output)
                    else:
                        # —— BFS：每个 root 产生可变/更多 step —— #
                        print(
                            f"[TRAIN][BFS] Detected BFS expansion: root_bs={root_bs}, "
                            f"gen_bs={gen_bs}, expected_n={expected_n}. "
                            f"Training directly on rollout batch.",
                            flush=True,
                            )
                    # 直接用 rollout 的批作为训练输入（要求 rollout 已提供 input_ids/attention_mask/response_mask/rm_scores）
                    ...
                    if rem != 0:
                        keep = N - rem
    # DataProto 支持切片；若你的版本不支持，就手动切 tensor/non-tensor
                    try:
                        batch = batch[:keep]
                    except Exception:
        # 手动裁剪（保底实现）
                        ...
                        #        从任一张量推断 bs
                            some_key = next(iter(td.keys()))
                            bs = int(td[some_key].shape[0])

# ===== 给 keep 一个安全默认值：不裁剪 =====
# 如果上游已经计算过 keep，就用上游的值；否则设置为 bs
                        try:
                            _ = keep  # 仅用于判断 keep 是否已存在
                        except NameError:
                                keep = bs
        ...
        # 手动裁剪（保底实现）
                                ...
                                for k, v in list(nt.items()):
                                    if isinstance(v, np.ndarray):
                                        ...
                                        nt[k] = v[:keep]
                                batch.batch = td
                                batch.non_tensor_batch = nt

                    # 统一（兜底）计算 response_mask；若 rollout 已给，这里为 no-op
                    batch.batch["response_mask"] = compute_response_mask(batch)
                    if int(batch.batch["response_mask"].sum().item()) == 0:
                        print(f"[WARN][skip-step] step={self.global_steps} has empty batch from rollout; "
                            f"sum(response_mask)=0, bs={len(batch)}. Skip this step.")
                        progress_bar.update(1)
                        self.global_steps += 1
                        continue
                    # 可选：跨 DP rank 均衡 valid tokens 数

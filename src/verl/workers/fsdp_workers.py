...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
        ...
        elif rollout_name in ["sglang", "sglang_async", "self_play"]:
            ...
            # --- BEGIN PATCH: 让 Worker 能 import 到项目根的 my_rollout.py ---
            import sys, os
            sys.path.insert(0, os.path.expanduser("/home/zss/Social_Behavior_Simulation"))
            from my_rollout import SelfPlayRollout
            # --- END PATCH ---
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # -------------------- PATCH: 识别“全局无样本跳过”并避免推进 LR -------------------- #
            if isinstance(metrics, dict) and metrics.get("actor/skip", 0) == 1:
                # 这里不做 mfu / lr 的计算，也不推进 lr_scheduler，保持与真实训练步一致
                # 依然做 postprocess / offload，返回 metrics 供上层记录
                ...
                if self._is_offload_param:
                    offload_fsdp_model_to_cpu(self.actor_module_fsdp)
                    log_gpu_memory_usage("After offload actor model during update_actor(skip)", logger=logger)
                if self._is_offload_optimizer:
                    offload_fsdp_optimizer(optimizer=self.actor_optimizer)
                    log_gpu_memory_usage("After offload actor optimizer during update_actor(skip)", logger=logger)
                return output
            # -------------------- PATCH END -------------------- #
